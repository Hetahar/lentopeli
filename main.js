...
// Funktio tason vaihtamiseen
async function levelChange () {
    try {
        if (gRoundNumber-1 >= countries.length) {
            return;
        }
...
// Sitten siittytään muihin tarkistuks vaiheisiin ja itse pelin päivittämiseen.
async function navIfCorrect(correctCountry) {
    ...
    document.getElementById('round_number').innerText = gRoundNumber // kierrosnumero
    gCurrentCountry = gNextCountry
    document.getElementById('current_country').innerText = gCurrentCountry; // tämänhetkinen valtio
...
nextLevel.addEventListener('click', async function(event) {
    ...
    if (await countryExists(submittedCountry)) {
        ...
        if (gCurrentCountry === "islanti" || gCurrentCountry === "italia" || gCurrentCountry === 'espanja') {
        stopTime = setInterval(function() {
            getTheLocalTime("ranska");
        }, 1000);
        } else {
            stopTime = setInterval(function() {
                getTheLocalTime(gCurrentCountry);
            }, 1000);
        }
